...
void sceneManager::release()
{
	...
	mapSceneIter miLoadingSceneList = _mLoadingSceneList.begin();

	for (; miLoadingSceneList != _mLoadingSceneList.end(); )
	{
		//지워지면 반복자를 증가하지 않는다.
		if (miLoadingSceneList->second != NULL)
		{
			if (miLoadingSceneList->second == _loadingScene) miLoadingSceneList->second->release();

			SAFE_DELETE(miLoadingSceneList->second);
			miLoadingSceneList = _mLoadingSceneList.erase(miLoadingSceneList);
		}
		else
		{
			++miSceneList;
